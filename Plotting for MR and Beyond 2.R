...
lm2 <- lm(performance~apt + scale(tt.skills), data=df) #running a new model that gives me an 
# intercept for aptitude centered at x[i] value 
lm2

lm1$coefficients #these values are used ro compete predicted performance holding tt at zero (allowing you to basically ignore it)
# Which basically allows you to do intercept + slope of apt minus x[1]

for (i in 1:length(xi)){

  df$apt.temp<-df$apt - xi[i] #my original aptitude measure minus the "i'th" value of x
  # lm<-lm(performance~apt.temp + tt.skills, data=df)
  lm<-lm(performance~apt.temp + scale(tt.skills), data=df) #re-centering estimating and plotting the intercept. This is optimal because in our model we're including test taking ability
